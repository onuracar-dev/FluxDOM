...
export function renderToString(source: string): string {
  ...
  try {
    ...
    const exports: any = {};
    const moduleFn = new Function('exports', 'require', 
      js.replace(/import .*? from .*?;/g, '') // strip imports for this basic eval
    );
